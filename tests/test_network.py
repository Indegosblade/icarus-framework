...
# fallback when no /AGENTS.md or /HANDOFF.md row exists. That fallback could
...
def test_privacy_stack_skips_project_observations_without_anchor(tmp_path):
    """No AGENTS.md/HANDOFF.md anywhere in the tree -> no valid anchor entity.
    ...
    assert first_file is not None and first_file[1] != "/AGENTS.md"


def test_privacy_stack_anchors_project_observations_to_real_file(tmp_path):
    """Control case: when AGENTS.md exists, observations anchor to its real
    ...
    src = tmp_path / "src"
    src.mkdir()
    (src / "AGENTS.md").write_text("# Project\nPi-hole and WireGuard notes.\n")
    (src / "scripts").mkdir()
    (src / "scripts" / "deploy.sh").write_text(
        "#!/bin/bash\n"
        "# password: hunter2page\n"
    ...
    conn = sqlite3.connect(str(db))
    try:
        agents_id = conn.execute(
            "SELECT id FROM files WHERE path = '/AGENTS.md'"
        ).fetchone()[0]
        anchor_ids = {
