...
- **Raised:** 54  •  **Verified real:** 51  •  **Fixed on `production-hardening`:** 11  •  **By design / documented:** 4  •  **Open backlog:** 36
...
### ◻️ open — changed_entities() uses SQL `!=`, which is NULL-blind and silently misses all changes to/from NULL
...
### ◻️ open — entitlement_diff diffs by autoincrement `id` across independent databases, masking real new entitlements
...
### ◻️ open — structural_diff joins on non-unique columns, producing Cartesian-product false-positive 'moved/reassigned' ch…
...
### ◻️ open — STIX SDOs omit required created/modified (and observed-data omits object_refs), producing spec-invalid bundles
...
### ◻️ open — diff_to_stix silently drops all 'changed' and 'structural' diff results
...
### ◻️ open — RAM-scaling PRAGMAs are applied only to the init connection that is immediately closed, so query/differ/resol…
...
### ◻️ open — foreign_keys enforcement is never enabled on any write connection — schema's REFERENCES are silently unenforc…
...
### ◻️ open — sqlite_parser leaks the source-DB connection on every unreadable/encrypted .db file
...
### ◻️ open — changed_entities uses `!=`, silently missing all NULL-to-value (and value-to-NULL) changes
...
### ◻️ open — CloudTrail observation dedup ignores the unique eventID and collapses distinct events
...
### ◻️ open — Conformance harness never validates event_types, and its schema check has a dead loop
...
### ◻️ open — Generic SQLite parser opens untrusted DBs read-write (mutates source, creates -wal/-shm, may run recovery) an…
...
### ◻️ open — DeployScripts parser uses DOTALL regexes with lazy quantifiers on attacker-controlled scripts (quadratic back…
...
### ◻️ open — Atom/Bag/EventLog resolver subsystem is fully built but never wired into the pipeline (dead code)
...
### ◻️ open — resolver.resolve() advertises threshold + FTS blocking it never uses; docstring overstates behavior
...
### ◻️ open — No curated public API surface; examples document a registration recipe that does not exist
...
### ◻️ open — EntityResolver.resolve() — the core resolution pipeline — has zero test coverage
...
### ◻️ open — HYGEIA built-in PII fallback is structurally guaranteed untested; same tests cover different code by environm…
...
### ◻️ open — Golden-output gate compares only row COUNTS, ignoring content and 3 of the golden file's own fields
...
### ◻️ open — Production parsers (linux + all generic/*) have golden files wired but no harness test ever runs them; linux …
...
### ◻️ open — Security gate (bandit) skips B608 SQL-injection — the exact bug class this f-string-heavy SQL codebase is pro…
...
### ◻️ open — Differ: removed/changed positive paths and 2 of 3 structural-diff branches are untested
...
### ◻️ open — STIX diff export drops all structural changes and its test asserts only the envelope
...
### ⚙️ by design — Checkpoint DB is never cleared after success, so re-running build on the same output is a silent no-op
...
### ⚙️ by design — Registry silently swallows ALL parser import failures (and lists 4 non-existent modules)
...
### ⚙️ by design — Parser registry silently swallows ImportError, hiding broken and phantom parsers
...
### ◻️ open — HYGEIA sanitize loads entire tables into memory via fetchall() — OOM on large databases
...
### ◻️ open — Parser registry silently swallows all manifest-load failures, degrading auto-detection
...
### ◻️ open — privacy_stack anchors all project-level observations to file id 1 on fallback
...
### ◻️ open — HYGEIA interpolates table/column names taken from sqlite_master into f-string SQL without the validators used…
...
### ◻️ open — Differ opens the 'new' database read-write and ATTACHes the 'old' one, both potentially untrusted
...
### ◻️ open — Implicit-Optional type hints combined with a mypy config that disables the checks that catch them
...
### ◻️ open — README claims '6 intelligence views' but the schema defines only 3
...
### ◻️ open — wiki Schema-Reference documents nonexistent 'versions' columns (uuid, created_at)
...
### ◻️ open — Typed package ships no py.typed marker (PEP 561)
...
### ◻️ open — Release metadata says '4 - Beta' while README calls v1.1.1 the stable/final release
...
### ⚙️ by design — parsers/__init__ registers four nonexistent parser modules
